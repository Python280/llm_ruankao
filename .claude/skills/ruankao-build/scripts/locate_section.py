...
BASE = os.getcwd()  # CWD 须为项目根（llm_ruankao）
IDX = json.load(open(os.path.join(BASE, "artifacts", "toc_index.json"), encoding="utf-8"))
PDF = IDX["pdf"]
...
def main():
    ...
    ap.add_argument("--out", help="摘录输出：纯文件名落 artifacts/ 下；带目录的相对路径基于 CWD；绝对路径照用")
    ...
    if a.extract:
        ...
        if not os.path.isabs(out) and not os.path.dirname(out):
            out = os.path.join(BASE, "artifacts", out)  # 纯文件名才落 artifacts/
